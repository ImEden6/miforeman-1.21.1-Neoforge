 ...
 * neoforge.mods.toml entry needed). If EMI isn't installed, nothing ever asks for this class, so
 * it's simply never loaded and its {@code dev.emi.*} imports never get resolved.
 */
@EmiEntrypoint
public class MiforemanEmiPlugin implements EmiPlugin {
    ...
    public void register(EmiRegistry registry) {
        ...
        // the right side genuinely free. A bounds provider covering just the panel is enough to
        // keep EMI's sidebar out of it. No exclusion area is needed, since there's nothing else
